...
{
    uint idx = get_local_id(0) * get_local_size(0) + get_local_id(1);

    data[idx] = val;
    barrier(CLK_LOCAL_MEM_FENCE);

    for (uint i = get_local_size(1) / 2; i > 0; i >>= 1)
    {
        if (get_local_id(1) < i)
        {
            data[idx] += data[idx + i];
...
__kernel void harris_response(
    ...
    __global const float* x_in,
    __global const float* y_in,
    const uint total_feat,
    __global uint* usable_feat,
    ...
    const uint patch_size)
{
    ...
    if (f < total_feat) {
        ...
        uint patch_r = ceil(size * sqrt(2.f) / 2.f);

        if (x >= patch_r && y >= patch_r && x < width - patch_r && y < height - patch_r) {
            uint r = block_size / 2;

            uint block_size_sq = block_size * block_size;
            for (uint k = get_local_id(1); k < block_size_sq; k += get_local_size(1)) {
                int i = k / block_size - r;
                int j = k % block_size - r;

                // Calculate local x and y derivatives
                float ix = read_imagef(image, sampler, (int2)(x+i+1, y+j)).x - read_imagef(image, sampler, (int2)(x+i-1, y+j)).x;
                float iy = read_imagef(image, sampler, (int2)(x+i, y+j+1)).x - read_imagef(image, sampler, (int2)(x+i, y+j-1)).x;
    ...
    if (f < total_feat && get_local_id(1) == 0) {
        uint idx = atomic_inc(usable_feat);
        if (idx < total_feat) {
            float tr = ixx + iyy;
    ...
    const uint patch_size)
{
    ...
    if (f < total_feat) {
        uint x = (uint)round(x_in[f]);
        uint y = (uint)round(y_in[f]);

        uint r = patch_size / 2;

        if (x >= r && y >= r && x <= width - r && y <= height - r) {
            uint patch_size_sq = patch_size * patch_size;
            for (uint k = get_local_id(1); k < patch_size_sq; k += get_local_size(1)) {
                int i = k / patch_size - r;
                int j = k % patch_size - r;

                // Calculate first order moments
				T p = read_imagef(image, sampler, (int2)(x+i, y+j)).x;
                m01 += j * p;
                m10 += i * p;
            }
        }
    ...
    const uint patch_size)
{
	...
	return read_imagef(image, sampler, (int2)(x, y)).x;
}

__kernel void extract_orb(
    __global uint* desc_out,
    const uint n_feat,
    __global float* x_in,
    __global float* y_in,
    ...
    const uint patch_size)
{
    ...
    if (f < n_feat) {
        ...
        y = (uint)round(y_in[f]);
        float ori = ori_in[f];
        uint size = patch_size;

        uint r = ceil(patch_size * sqrt(2.f) / 2.f);

        if (x >= r && y >= r && x < width - r && y < height - r) {
           // Descriptor fixed at 256 bits for now
            for (uint i = get_local_id(1); i < 16; i += get_local_size(1)) {
                uint v = 0;

                for (uint j = 0; j < 16; j++) {
                    int dist_x = ref_pat[i*16*4 + j*4];
                    int dist_y = ref_pat[i*16*4 + j*4+1];
                    T p1 = get_pixel(x, y, ori, size, dist_x, dist_y, image, height, width, patch_size);

                    dist_x = ref_pat[i*16*4 + j*4+2];
                    dist_y = ref_pat[i*16*4 + j*4+3];
                    T p2 = get_pixel(x, y, ori, size, dist_x, dist_y, image, height, width, patch_size);
